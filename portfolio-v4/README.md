...
- `Hero.astro`: an original torus wireframe is generated as SVG at build time, draws on entry and tilts subtly with the cursor. Below it, an oversized pink statement introduces three dark, illustrated approach panels linked to real projects. Their original SVG flower, layered grid and radial signal use native scroll-linked CSS, with static fallbacks and no additional client JavaScript. Reduced motion leaves all illustrations static.
