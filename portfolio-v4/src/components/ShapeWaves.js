/*! ShapeWaves by David Haz - MIT + Commons Clause; see ShapeWaves.LICENSE. */
...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  // Dissolve low-energy bands so the field reads as clouds, not a filled grid.
  level *= smoothstep(0.12, 0.78, tone);
...
export default function ShapeWaves(
  ...
  const handlePointerMove = (event) => {
    if (!options.interactive || reduceMotion.matches || event.pointerType === 'touch') return;
    ...
    if (inside) {
      ...
      splash(x, y, Math.min(1, 0.22 + speed * 0.0006) * options.splashStrength);
      wakeRenderer();
    }
    pointer.x = x;
    pointer.y = y;
  ...
  void (async () => {
    try {
      ...
      const glowEnabled = () => options.glow > 0;
      let lastIntro = false;
      ...
      applyCurrentSettings = applySettings;
